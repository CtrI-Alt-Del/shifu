...
| `ACH-05` | High | Fixed | The server integration job segfaulted in CI: the outbox listener thread was running `LISTEN` and `commit()` while `unlisten()` closed the same psycopg connection, and closing a connection with a command in flight crashes libpq. The stack is `events_repository_listener._run` to `psycopg._exec_command`. The race predates this delivery and lives in shared infrastructure; adding two controller suites made the window more likely to be hit. A lock now keeps the subscription command and the close from overlapping. It could not be reproduced on this machine, where Testcontainers Redis cannot map a port and skips those suites, so CI is the verification. |
