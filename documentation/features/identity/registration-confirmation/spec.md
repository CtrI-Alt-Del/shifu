...
| `RF-08` | Identity `RP-02`, `RP-10`, `JN-01` | Present distinct understandable valid, expired, already-used and invalid confirmation outcomes, with a safe recovery path and no secret left in browser storage or the settled URL. Only a matching opaque pending context, including one issued after restricted pending sign-in, may create a same-access session. |
...
| `CA-08` | `RF-07`, `RF-08` | Valid one-time confirmation | A pending account opens its current token before 24 hours | Confirmation commits | Account becomes active once, confirmation time is stored, remaining tokens are invalidated and the URL no longer contains the token; only the browser holding the matching opaque context, including after restricted sign-in, receives a session and continues to `/`; every other browser reaches `/login` without a session | Use-case/controller/handler/route tests and `VM-05` |
...
| `POST /identity/sign-in` | Existing credentials contract | Existing authentication response plus `pending_handle:string(base64url,43 chars)` only for `activation-only` | Identity replaces the current pending confirmation handle atomically after validating restricted credentials. The BFF stores it only in the opaque HttpOnly pending context. |
...
| `POST /identity/pending-confirmations/validate` | `{pending_handle:string(base64url,43 chars),account_id:string}` | `200 {valid:boolean}` | The BFF uses this internal validation after activation. It returns true only when the handle belongs to the just-used confirmation token and the same account is active; it reveals no account state to the browser. |
...
| `apps/server/src/shifu/identity/core/use_cases/issue_pending_confirmation_context_use_case.py` | Create | `IssuePendingConfirmationContextUseCase.execute` | After restricted pending sign-in, atomically replaces the current eligible confirmation token's pending-handle hash with a new opaque handle. | Sign-in controller and pending BFF context | Focused unit/controller tests |
| `apps/server/src/shifu/identity/core/use_cases/verify_pending_confirmation_context_use_case.py` | Create | `VerifyPendingConfirmationContextUseCase.execute` | Validates that an opaque pending handle belongs to the just-used confirmation token for the activated account, without exposing state. | Confirmation BFF session promotion | Focused unit/controller tests |
...
| `apps/server/src/shifu/identity/rest/controllers/verify_pending_confirmation_context_controller.py` | Create | `POST /identity/pending-confirmations/validate` | Provides BFF-only same-browser session correlation after activation. | Pending-context verification use case | REST integration |
...
| `apps/server/tests/identity/core/use_cases/test_sign_in_use_case.py` | Modify | Sign-in normalization regression coverage | Proves whitespace/case-insensitive e-mail lookup while preserving generic rejection and no protected side effects | `AuthCredentials`, `SignInUseCase` | `CI-15` |
...
| --- | --- | --- | --- |
| `apps/server/tests/identity/core/use_cases/test_register_account_use_case.py` | Unit | Registration use case | Validation, normalization, privacy result, hashing, transaction and initial token |
| `apps/server/tests/identity/core/use_cases/test_confirm_account_use_case.py` | Unit | Confirmation use case | Valid, expired, used, invalidated and concurrent consumption |
| `apps/server/tests/identity/core/use_cases/test_resend_email_confirmation_use_case.py` | Unit | Resend use case | Generic eligibility, 60-second boundary, invalidation and concurrency |
| `apps/server/tests/identity/core/use_cases/test_expire_unconfirmed_accounts_use_case.py` | Unit | Expiry use case | Seven-day boundary, immutable deadline, idempotency and access invalidation |
| `apps/server/tests/identity/core/use_cases/test_record_communication_delivery_state_use_case.py` | Unit | Identity delivery-state use case | Correlation, allowed terminal transitions, duplicate events and privacy-safe persistence |
| `apps/server/tests/communication/core/use_cases/test_queue_communication_use_case.py` | Unit | Queue use case | Catalog validation, request idempotency, encrypted content handoff and outbox timing |
| `apps/server/tests/communication/core/use_cases/test_deliver_communication_use_case.py` | Unit | Delivery use case | Attempt state, orphaned processing recovery, temporary/permanent mapping, retry schedule and terminal reporting |
| `apps/server/tests/communication/core/use_cases/test_cancel_communication_use_case.py` | Unit | Communication cancellation use case | Persisted ID-pair correlation, `confirmed`/`reissued` stop behavior, `expired` redaction across pending/delivered/terminal rows, retained metadata and duplicate-event idempotency |
| `apps/server/tests/identity/server/controllers/test_register_account_controller.py` | Integration | Registration HTTP/persistence | Real PostgreSQL account/token/outbox state, duplicate race and safe response |
| `apps/server/tests/identity/server/controllers/test_confirm_account_controller.py` | Integration | Confirmation HTTP/persistence | Atomic activation/consumption, statuses and rollback |
| `apps/server/tests/identity/server/controllers/test_resend_email_confirmation_controller.py` | Integration | Resend HTTP/persistence | Cooldown, replacement, invalidation, queue failure and safe response |
| `apps/server/tests/identity/server/controllers/test_get_pending_confirmation_status_controller.py` | Integration | Pending-status HTTP/persistence | Handle validation, real/decoy/expired mapping and privacy-safe response through TestClient/PostgreSQL |
...
| 12 | 2026-09-23 | Returned the local contract to `draft` for PR #9 corrections: pending-context correlation after restricted sign-in, same-browser-only session promotion, recoverable registration handoff, orphaned delivery settlement, module-first test locations and required CI artifacts. | The PRD authorities remain Identity `83001345` v1 and Communication `86114306` v1; the corrections preserve their outcomes while repairing local technical contracts and evidence boundaries. |
