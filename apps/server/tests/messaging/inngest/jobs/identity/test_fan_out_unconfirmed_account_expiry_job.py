...
from shifu.identity.core.domain.events import AccountExpiryRequestedEvent
...
class TestFanOutUnconfirmedAccountExpiryJob:
    def test_cron_fans_out_bounded_independent_children_and_is_idempotent(
        ...
        _insert_expired_pending_accounts(inngest_fixture, count=101)

        inngest_fixture.invoke_function(_FUNCTION_ID)
        inngest_fixture.wait_for_database(
            lambda session: _count_accounts_with_status(session, 'deleted') == 100,
            timeout=300.0,
        )

        with inngest_fixture.inspection_session() as session:
            assert _count_accounts_with_status(session, 'pending-confirmation') == 1
            assert _count_expiry_requested_events(session) == 100

        inngest_fixture.invoke_function(_FUNCTION_ID)
        inngest_fixture.wait_for_database(
            lambda session: _count_accounts_with_status(session, 'deleted') == 101
        )

        with inngest_fixture.inspection_session() as session:
            assert _count_expiry_requested_events(session) == 101
        ...
        inngest_fixture.invoke_function(_FUNCTION_ID)
        inngest_fixture.wait_for_database(
            lambda session: _count_expiry_requested_events(session) == 101
        )

        with inngest_fixture.inspection_session() as session:
            assert _count_accounts_with_status(session, 'deleted') == 101
            assert _count_expiry_requested_events(session) == 101
...
def _count_expiry_requested_events(session: Session) -> int:
    return int(
        session.scalar(
            ...
            .where(EventModel.name == AccountExpiryRequestedEvent.name)
        )
        or 0
    )
