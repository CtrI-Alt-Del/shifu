...
def serialize_value(value: object) -> object:
    ...
    if isinstance(value, dict):
        ...
        return {str(key): serialize_value(item) for key, item in items.items()}
    return value
...
def deserialize_value(value: object, target_type: object) -> object:  # noqa: C901
    ...
    if origin in (types.UnionType,):
        ...
        for argument in arguments:
            if argument is type(None):
                continue
            try:
                return deserialize_value(value, argument)
            except (KeyError, TypeError, ValueError) as error:
                errors.append(error)
        raise TypeError(f'Unable to deserialize {value!r} as {target_type!r}') from (
    ...
    if origin is tuple:
        ...
        return tuple(
            deserialize_value(item, item_type) for item in cast('list[object]', value)
        )
    ...
    if origin is dict:
        ...
        return {
            deserialize_value(key, key_type): deserialize_value(item, value_type)
    ...
    if isinstance(target_type, type) and is_dataclass(target_type):
        ...
        return target_type(
            **{
                field.name: deserialize_value(data[field.name], type_hints[field.name])
