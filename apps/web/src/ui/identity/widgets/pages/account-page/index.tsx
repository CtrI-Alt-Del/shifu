...
const accountSections = [
  {
    description: 'Nome, e-mail e segurança da sua conta.',
    status: 'Em preparação',
    title: 'Dados da conta',
  },
  {
    description: 'Ajuste o jeito que o Shifu acompanha seu ritmo.',
    status: 'Em preparação',
    title: 'Preferências de aprendizagem',
  },
  {
    description: 'Entenda como seus dados participam da experiência.',
    status: 'Disponível na próxima etapa',
    title: 'Privacidade',
  },
] as const
  ...
  return (
    <div className='space-y-10'>
      ...
      <section className='flex flex-col gap-6 rounded-3xl border border-border bg-card p-7 shadow-card sm:flex-row sm:items-center sm:p-9'>
        <div className='grid size-20 shrink-0 place-items-center rounded-3xl bg-accent font-serif text-3xl font-bold text-primary'>
          AS
        </div>
        <div>
          ...
          <p className='mt-2 text-muted-foreground'>
            Seu perfil está pronto para ganhar contexto ao longo da jornada.
          </p>
        </div>
      </section>
        ...
        <div className='mt-5 grid gap-4 md:grid-cols-3'>
          {accountSections.map((section) => (
            ...
            >
              ...
              <p className='mt-6 text-xs font-bold uppercase tracking-[0.1em] text-primary'>
                {section.status}
              </p>
            </article>
          ))}
